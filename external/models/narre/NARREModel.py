...
class NARREModel(tf.keras.Model, ABC):
    ...
    def predict(self, batch):
        ...
        u_feas = self.forward_user_embeddings((user, user_reviews, pos_user), training=False)
        i_feas = self.forward_item_embeddings((item, item_reviews, pos_item), training=False)
        u_bias = tf.gather(self.uidW2, user)
        i_bias = tf.gather(self.iidW2, item)
        xui = self(inputs=(u_feas, u_bias, i_feas, i_bias), training=False)
        return xui
