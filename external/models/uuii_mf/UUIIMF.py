...
from .UUIIMFModel import UUIIMFModel
...
class UUIIMF(RecMixin, BaseRecommenderModel):
    @init_charger
    ...
    def get_recommendations(self, k: int = 100):
        ...
        with tqdm(total=int(val_len // self._batch_eval), disable=not self._verbose) as t:
            for index, offset in enumerate(range(0, val_len, self._batch_eval)):
                ...
                p = self._model.predict(gu, gi, current_df['user'].tolist(), current_df['item'].tolist())
